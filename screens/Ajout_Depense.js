...
export default function Ajout_Depense({ route, navigation }) {
  const { addTransaction, SommeDepMensuelle, SommeRevMensuel } =
    useTransactions();
  ...
  const get_Date = (_date) => {
    let date_temp = `${_date.getFullYear()}-${
      _date.getMonth() + 1
    }-${_date.getDate()}`;
    return date_temp;
  };
  ...
  return (
    <View style={Styles.container}>
      ...
      <View style={[Styles.box, Styles.depense]}>
        ...
        <Text style={{ marginBottom: 2, fontSize: 18, color: "#747264" }}>
          {categorie}
        </Text>
      </View>
      ...
      <View style={{ marginTop: 20 }}>
        <Button
          title={"Enregistrer"}
          onPress={async () => {
            if (SommeRevMensuel < SommeDepMensuelle + parseInt(montant)) {
      ...
      >
        <View style={modalStyles.centeredView}>
          <View style={modalStyles.modalView}>
            <Text style={modalStyles.modalText}>Pas Assez de revenu !!</Text>
